# Vied Dental — Clone & Deploy (the reliable one-shot)
...
**Source of truth (public):** `https://github.com/toprmrproducer/Vied-dental`
**Live reference (what the result must look like):** `https://toprmrproducer.github.io/Vied-dental/`
...
in, the `ViedLead` lead-form handler, and the image guard). There is no `app.js` and no build output.
...
```bash
git clone https://github.com/toprmrproducer/Vied-dental.git
cd Vied-dental
```
...
- Browser: open `https://github.com/toprmrproducer/Vied-dental/archive/refs/heads/main.zip`, unzip,
  and `cd` into the unzipped `Vied-dental-main` folder.
- macOS/Linux one-liner: `curl -L -o Vied.zip https://github.com/toprmrproducer/Vied-dental/archive/refs/heads/main.zip && unzip Vied.zip && cd Vied-dental-main`
- Windows PowerShell: `Invoke-WebRequest https://github.com/toprmrproducer/Vied-dental/archive/refs/heads/main.zip -OutFile Vied.zip; Expand-Archive Vied.zip -Force; cd Vied-dental-main\Vied-dental-main`
...
1. On GitHub, **Fork** `toprmrproducer/Vied-dental` into your own account (button top-right), OR
...
stay exactly as-is. Do these edits in a code editor across all `.html` files (and `assets/css/Vied.css`
...
7. **Color:** the theme is driven by CSS custom properties. In `assets/css/Vied.css`, change the
